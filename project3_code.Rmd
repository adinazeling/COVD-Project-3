  ...
  while(delta > 1e-4 && iter <= itermax){
    ...
    for (i in 1:k) { #sum of p Euclidian distances from k centroids per obs
      dist <- (map2_df(X, centroid[i,], `-`))^2 %>%
        mutate(dist = A + B + C) %>%
        dplyr::select(dist)
      distance <- bind_cols(distance, dist)
    }
    ...
    for (i in 1:k) { # Compute new k*p centroids
      mean <- X[cluster == i,] %>% summarise(B = mean(B), A = mean(A), C = mean(C))
...
km_vec <- cbind(cases_country_curves, cluster = km$cluster)
covid19_select <- covid19 %>% 
  rename(Country = 'Country/Region') %>%
  dplyr::select(c(Country, Lat, Long))
  
test <- inner_join(km_vec, covid19_select) %>% distinct()
