...
new_covid = new_covid %>% 
  ...
  summarise(Confirmed = sum(Confirmed),
            Deaths = sum(Deaths)) %>% 
  filter(Confirmed != 0) %>%
  mutate(Date = as.Date(Last_Update, format="%m/%d/%Y")) %>%
...
for (i in dim(par_new)[1]) {
  data = gen_cases(par_new[i,1], par_new[i,6], par_new[i,7], par_new[i,8])
  predict_case = rbind(predict_case, data)
}
```
