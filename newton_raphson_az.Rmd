...
Fisher = function(t, y, start, tol = 1e-10, maxiter = 200){
  ...
  while (i < maxiter && abs(currloss - prevloss) > tol) {
    ...
    # Next step of log likelihood
    cur = prev - step * inv(stuff$info) %*% stuff$grad
    ...
    # Ascent direction check
    ascent_dir = -t(stuff$grad) %*% inv(stuff$info) %*% stuff$grad
    if (ascent_dir < 0) {
    ...
    if (currloss > prevloss) {
      ...
      while (currloss < prevloss) {
        step = step * (1/2)
        cur = prev - step * inv(info_new) %*% stuff$grad
        stuff = func(t, y, cur)
        currloss = stuff$loss
        }
...
```{r}
test1 = Fisher(NY_dat$time_from_first_case, NY_dat$ConfirmedCases, c(20, 10, 10), maxiter = 1000)
test2 = NR_modify(NY_dat$time_from_first_case, NY_dat$Fatalities, c(1, 1, 1))
